...
document.addEventListener('DOMContentLoaded', () => {
    ...
    */
                    ...
                    <div style="padding:40px 40px 20px;flex:1;overflow-y:auto;" id="modal-scroll-body">
                        ...
                        <div id="gateway-selection-row" style="margin-bottom:10px;">
                            ...
                            <div style="display:grid;grid-template-columns:1fr 1fr;gap:15px;">
                                <label id="label-rzp" style="background:rgba(124,58,237,0.1);border:1px solid #7c3aed;padding:16px;border-radius:14px;cursor:pointer;text-align:center;transition:all 0.2s;display:flex;flex-direction:column;gap:4px;">
                                    <input type="radio" name="gateway" value="razorpay" checked style="display:none;">
                                ...
                                <label id="label-cf" style="background:rgba(255,255,255,0.02);border:1px solid rgba(255,255,255,0.1);padding:16px;border-radius:14px;cursor:pointer;text-align:center;transition:all 0.2s;display:flex;flex-direction:column;gap:4px;">
                                    <input type="radio" name="gateway" value="cashfree" style="display:none;">
                                    <span style="color:white;font-size:14px;font-weight:700;">CASHFREE</span>
                                    <span style="color:rgba(255,255,255,0.3);font-size:9px;">ALTERNATIVE</span>
                                </label>
                            </div>
                        </div>
    ...
    function selectGateway(gatewayName) {
        const isCF = gatewayName === 'cashfree';
        const labelCF = document.getElementById('label-cf');
        const labelRZP = document.getElementById('label-rzp');
        ...
        if (isCF) {
            ...
            if (rzpRadio) rzpRadio.checked = false;
            ...
            if (badgeText) badgeText.textContent = 'Secured by Cashfree • 256-bit SSL Encryption';
        } else {
            if (rzpRadio) rzpRadio.checked = true;
            if (cfRadio) cfRadio.checked = false;
            if (labelRZP) { labelRZP.style.background = 'rgba(124,58,237,0.1)'; labelRZP.style.borderColor = '#7c3aed'; }
            if (labelCF) { labelCF.style.background = 'rgba(255,255,255,0.03)'; labelCF.style.borderColor = 'rgba(255,255,255,0.1)'; }
            if (badgeText) badgeText.textContent = 'Secured by Razorpay • Instant Settlements';
        }
    }

    const labelRZP = document.getElementById('label-rzp');
    const labelCF = document.getElementById('label-cf');
    if (labelRZP) labelRZP.addEventListener('click', () => selectGateway('razorpay'));
    if (labelCF) labelCF.addEventListener('click', () => selectGateway('cashfree'));

    const gatewayRadios = document.querySelectorAll('input[type="radio"][name="gateway"]');
    gatewayRadios.forEach(radio => {
        radio.addEventListener('change', () => selectGateway(radio.value));
    });
    ...
    document.addEventListener('submit', async (e) => {
        ...
        const gateway = gatewayInput ? gatewayInput.value : 'razorpay';
        ...
        if (selectedPlanType === 'monthly') {
            try {
                ...
                // Fetch dynamic plan ID and current sub price from config/pricing
                let planId = null;
        ...
        if (gateway === 'razorpay') {
            ...
            if (window.userCountryCode === 'PK' || phoneVal.startsWith('+92')) {
                showToast('Sorry, we are unable to accept payments from your region at this time.', 'error');
                payBtn.disabled = false;
                btnText.style.display = 'inline';
                btnLoader.style.display = 'none';
                return;
            ...
            try {
                ...
                if (err.message === 'Razorpay SDK not loaded') {
                    showToast('Payment gateway failed to load. Please disable adblock or try a different browser.', 'error');
                } else {
                    showToast('Razorpay is currently unavailable for your region or details. Please contact support.', 'error');
                }
                payBtn.disabled = false;
                btnText.style.display = 'inline';
                btnLoader.style.display = 'none';
        ...
        // ── International Logic: Alternative checkout disabled ──
        if (currency === 'USD') {
            showToast('Alternative international checkout is currently disabled. Please use Razorpay.', 'error');
            payBtn.disabled = false;
            btnText.style.display = 'inline';
            btnLoader.style.display = 'none';
            return;
